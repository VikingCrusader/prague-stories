...
Era 1 (`legends-origins`) is content-complete and live (all 7 events, sorted by `startYear`: `early-history`, `libuse-prophecy`, `premysl-the-ploughman`, `founding-of-vysehrad`, `girls-war`, `st-ludmila-martyrdom`, `st-wenceslas-murder`). Copy was rewritten in a lighter, "storybook" narrative voice — each event's opening line deliberately picks up from the previous card's ending (e.g. `libuse-prophecy` opens "That legend starts, as all the best ones do...", directly following `early-history`'s closing line) so the feed reads as one continuous story, not seven separate trivia entries. Applies across `en`/`cz`/`zh`; `zh` still follows the no-parenthetical-foreign-name-gloss rule from **Adding new history events** above, though a period/movement term (like "Palaeolithic") explained inline in the reader's own language is fine and was deliberately added back into `early-history` after an earlier pass had trimmed it for brevity.

**Illustrations — 5 of 7 events have one, 2 remaining:**
- Done: `early-history` → `/history/prehistory.webp`, `libuse-prophecy` → `/history/libuse.webp`, `premysl-the-ploughman` → `/history/premysl.webp`, `founding-of-vysehrad` → `/history/vysehrad.webp`, `girls-war` → `/history/sarka.webp`
- **Still needed:** `st-ludmila-martyrdom` and `st-wenceslas-murder` have no `image` set. Once generated, drop the file in `client/public/history/` (suggest `ludmila.webp` / `wenceslas.webp` to match the existing short-name convention) and set each event's `image` field to `/history/<file>.webp` — both in `seedHistoryEvents.js` *and* live via `HistoryEvent.updateOne({ slug }, { $set: { image } })`, since the seed script's `$setOnInsert` won't touch already-seeded docs (see **Adding new history events** above).
- **Working image-gen prompt template**, refined through several iterations this session — reuse it rather than starting from scratch: wide 16:9 (1024×576), aged tan parchment/scroll border with Celtic-knot corner flourishes, painterly high-detail pixel art (not blocky 8-bit), a single wide panoramic scene (not a "vision-within-a-vision" inset — that read as visually out of place and got dropped), and a bottom caption plaque described explicitly as *"a simple, perfectly flat rectangular parchment-colored bar... straight parallel edges... not tilted or in perspective"* (image models otherwise render it with an odd warped/perspective tilt) holding a **short** bold serif all-caps quote, with an explicit instruction to render it once with no repeated words (longer two-clause captions were prone to a word-duplication glitch). See the conversation this shipped in for the exact prompt text used for `libuse.webp` and `sarka.webp` as copyable references.
